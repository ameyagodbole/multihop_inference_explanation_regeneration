...
def compute_metrics(examples, preds, labels):
    ...
    precision = sum((predicted_labels != 0) & (labels != 0)) / sum(predicted_labels != 0)
    recall = sum((predicted_labels != 0) & (labels != 0)) / sum(labels != 0)

    return {"acc": acc, "precision": precision, "recall": recall}, None
...
class TG2019RerankProcessor:
    def get_train_examples(self, args):
        ...
        return self._create_examples(df_questions, args.mcq_choices, args.facts_file, args.adjacency_list_file,
                                     args.tfidf_top_k,)
    ...
    def get_test_examples(self, args):
        ...
        return self._create_examples(df_questions, args.mcq_choices, args.facts_file, args.adjacency_list_file,
                                     args.tfidf_top_k, test=True)

    def get_labels(self):
        """Gets the list of labels for this data set."""
        return ["0", "1"]

    def _create_examples(self, df_questions, mcq_choices, facts_file, adjacency_list_file, tfidf_top_k, test=False):
        ...
        adjacency_list = pickle.load(open(adjacency_list_file, "rb"))
        ...
        for i_q, question in df_questions.iterrows():
            ...
            linked_uids = [df_facts.loc[fid, ["uid", "text"]] for fid in np.argsort(X_dist[i_q])[:tfidf_top_k]]
            for uid, fact in linked_uids:
                if uid in added_pairs:
                    continue
                added_pairs.add((uid, uid))
                guid = '###'.join([qid, uid])
                examples.append(InputExample(guid=guid, text_a=question["ProcessedQuestion"], text_b=[fact],
                                             label="1" if uid in explanations else "0"))
                if uid not in adjacency_list:
                    continue
                for neighbour_uid in adjacency_list[uid]:
                    if (uid, neighbour_uid) in added_pairs:
                        continue
                    added_pairs.add((uid, neighbour_uid))
                    guid = '###'.join([qid, uid, neighbour_uid])
                    examples.append(InputExample(guid=guid, text_a=question["ProcessedQuestion"],
                                                 text_b=[fact,
                                                         df_facts.loc[df_facts["uid"] == neighbour_uid, "text"].item()],
                                                 label="1" if (
                                                         uid in explanations and neighbour_uid in explanations) else "0"))
...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens_b = tokenizer.tokenize('. '.join(example.text_b))
...
def load_and_cache_examples(args, task, tokenizer, mode='train', return_sampler_weights=False):
    ...
    cached_examples_file = os.path.join(args.data_dir, 'bert_path{}_rank_cached_examples_{}_{}_{}_{}_{}'.format(
        args.max_path_length,
        mode,
        list(filter(None, args.model_name_or_path.split('/'))).pop(),
        str(args.max_seq_length),
        str(args.tfidf_top_k),
        str(task)))
    if args.cached_examples_file:
        cached_examples_file = args.cached_examples_file
    cached_features_file = os.path.join(args.data_dir, 'bert_path{}_rank_cached_features_{}_{}_{}_{}_{}'.format(
        args.max_path_length,
        mode,
        list(filter(None, args.model_name_or_path.split('/'))).pop(),
        str(args.max_seq_length),
    ...
    if output_mode == "classification":
        ...
        raise ValueError("Unhandled output mode {}".format(output_mode))
...
def main():
    ...
    parser.add_argument("--adjacency_list_file", default="", type=str,
                        help="The pickle file containing the adjacency list of facts")
    ...
    parser.add_argument("--tfidf_top_k", type=int, default=25,
                        help="Number of top TF-IDF facts to expand")
    ...
    parser.add_argument("--max_path_length", default=2, type=int,
                        help="The maximum total path length to explore in the graph")
